...
#define         vlin(va, n)  for(int i=0; i<n; i++){lli x;cin>>x;va.PB(x);}
...
//string ReadLine() {string s;char ch = getchar();while (ch != '\n') {s += ch;ch = getchar();}return s;}
...
STL/Bitmasks/Prefix Sum/Binary Exponentiation/DFS
/BFS/Binary Search/Recursion/GCD/LCM/Divisors/Knapsack/LCS
