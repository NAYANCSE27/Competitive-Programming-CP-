...
int main() {
    ...
    while(t--) {
        ...
        for(int i=0; i<n; i++) {
            for(int j=i+2; j<n; j++) {
                lli mid=(va[i]+va[j]+1)/2;
                vector<lli>::iterator lower,upper;
                lower=lower_bound(va.begin(), va.end(), mid);
                //upper=upper_bound(va.begin(), va.end(), mid);

                int x=lower-va.begin();
                //int y=upper-va.begin();
                mid=-1e8;
                if(x!=j){
                    lli bal=((va[j]-va[x])*(va[x]-va[i]));
                    mid=max(mid, bal);
                }
                x--;
                if(x!=i){
                ...
                //lli mm=(va[i]-va[x])*(va[x]-va[j]);
                //lli mn=(va[i]-va[y])*(va[y]-va[j]);
                //cout<<"\t"<<max(mm,mn)<<endl;
                total+=mid;
            }
        }
